...
- `src/skulk/operator/` — Stable operator identity and encrypted authority-store
  foundation. It is a separate security plane from event-sourced inference
  state; do not place its secrets or mutable authorization records in `State`,
  telemetry, diagnostics, or ordinary events.
