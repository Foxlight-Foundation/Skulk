...
export function ChatSidebar({
  ...
  const filtered = useMemo(() => {
    ...
    return conversations.filter((c) => conversationDisplayName(c.name, t).toLowerCase().includes(q));
    ...
    <Sidebar className={className}>
      <Header>
        <Button variant="primary" size="md" block onClick={onNewChat}>
          {t('chat.sidebar.newChat', '+ New Chat')}
        </Button>
      </Header>
      ...
      <SectionLabel>
        {searchQuery
          ? t('chat.sidebar.searchResults', 'Search Results')
          : t('chat.sidebar.conversations', 'Conversations')}
      </SectionLabel>

      <List>
        {filtered.length === 0 ? (
          <EmptyState>
            {searchQuery ? t('chat.sidebar.noResults', 'No results') : t('chat.sidebar.noConversations', 'No conversations')}
          </EmptyState>
        ) : (
          filtered.map((conv) => (
            <ConvItem
            ...
            >
              {editingId === conv.id ? (
                <div>
                  ...
                  />
                  <BtnRow>
                    <Button variant="primary" size="sm" onClick={(e) => { e.stopPropagation(); saveRename(); }}>
                      {t('common.save', 'Save')}
                    </Button>
                    <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); setEditingId(null); }}>
                ...
                <ConfirmBox $danger onClick={(e) => e.stopPropagation()}>
                  {t('chat.sidebar.deleteConversationConfirm', 'Delete "{name}"?', {
                    name: conversationDisplayName(conv.name, t),
                  })}
                  <BtnRow>
                    <Button variant="danger" size="sm" onClick={(e) => { e.stopPropagation(); confirmDelete(conv.id); }}>
                ...
                <>
                  <ConvName $active={conv.id === activeId}>{conversationDisplayName(conv.name, t)}</ConvName>
                  <ConvDate>{formatDate(conv.updatedAt, t)}</ConvDate>
                  <HoverActions>
                    <Button
                      variant="ghost"
                      size="sm"
      ...
      <Footer>
        {showDeleteAll ? (
          <ConfirmBox $danger>
            {t('chat.sidebar.deleteAllConfirm', 'Delete all {count} conversations?', { count: conversations.length })}
            <BtnRow>
              <Button variant="danger" size="sm" onClick={() => { onDeleteAllConversations(); setShowDeleteAll(false); }}>
                {t('chat.sidebar.deleteAll', 'Delete All')}
              </Button>
          ...
          <Button variant="danger" size="sm" onClick={() => setShowDeleteAll(true)}>
            ...
            {t('chat.sidebar.deleteAllChats', 'Delete all chats')}
          </Button>
        ) : null}
        <ConvCount>
          {t('chat.sidebar.conversationCount', '{count} conversation{plural}', {
            count: conversations.length,
            plural: conversations.length !== 1 ? 's' : '',
          })}
        </ConvCount>
      </Footer>
    </Sidebar>
