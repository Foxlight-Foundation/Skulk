...
8. We want people to be able to copy models over to a new device without ever connecting Skulk to the internet. Right now Skulk requires internet connection once to cache some files to check if a download is complete. Instead, we should simply check if there is a non-empty model folder locally with no .partial files. This indicates it's a fully downloaded model that can be loaded.
