...
from skulk.worker.engines.mlx.turboquant import (
    make_turboquant_adaptive_cache,
    make_turboquant_cache_from_template,
)
from skulk.worker.engines.mlx.turboquant.cache import ensure_standard_attention
...
_MEMORY_THRESHOLD = float(
    os.environ.get("SKULK_MEMORY_THRESHOLD", _default_memory_threshold())
)
...
def make_kv_cache(
    ...
    if backend == "mlx_quantized":
        if KV_CACHE_BITS is None:
            raise ValueError(
                "SKULK_KV_CACHE_BACKEND=mlx_quantized requires SKULK_KV_CACHE_BITS to be set"
            )
        logger.info(
    ...
    if backend == "optiq":
        try:
            ...
            raise RuntimeError(
                "SKULK_KV_CACHE_BACKEND=optiq requires mlx-optiq to be installed. "
...
def get_kv_cache_backend() -> KVCacheBackend:
    ...
    if backend not in VALID_KV_CACHE_BACKENDS:
        logger.warning(
            f"Unknown SKULK_KV_CACHE_BACKEND={backend!r}; falling back to {DEFAULT_KV_CACHE_BACKEND!r}"
        )
