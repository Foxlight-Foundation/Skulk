...
from skulk.worker.engines.mlx.generator.generate import mlx_generate, prefill
from skulk.worker.engines.mlx.utils_mlx import apply_chat_template
from skulk.worker.tests.unittests.test_mlx.conftest import (
...
class TestKVCacheBackends:
    def test_unknown_backend_falls_back_to_default(self):
        with patch(
            "skulk.worker.engines.mlx.cache.KV_CACHE_BACKEND",
            cast(object, "mystery"),
        ):
    ...
    def test_make_kv_cache_default_backend(self):
        ...
        with patch("skulk.worker.engines.mlx.cache.KV_CACHE_BACKEND", "default"):
    ...
    def test_make_kv_cache_mlx_quantized_backend(self):
        model = cast(Model, type("FakeModel", (), {"layers": [object(), object()]})())
        with (
            patch("skulk.worker.engines.mlx.cache.KV_CACHE_BACKEND", "mlx_quantized"),
            patch("skulk.worker.engines.mlx.cache.KV_CACHE_BITS", 4),
            patch("skulk.worker.engines.mlx.cache.CACHE_GROUP_SIZE", 32),
        ):
            cache = make_kv_cache(model)
    ...
    def test_make_kv_cache_mlx_quantized_backend_requires_bits(self):
        ...
        with (
            patch("skulk.worker.engines.mlx.cache.KV_CACHE_BACKEND", "mlx_quantized"),
            patch("skulk.worker.engines.mlx.cache.KV_CACHE_BITS", None),
            pytest.raises(ValueError, match="SKULK_KV_CACHE_BITS"),
        ):
    ...
    def test_make_kv_cache_turboquant_backend(self):
        ...
        model = cast(Model, FakeModel())
        with (
            patch("skulk.worker.engines.mlx.cache.KV_CACHE_BACKEND", "turboquant"),
            patch("skulk.worker.engines.mlx.cache.TURBOQUANT_K_BITS", 3),
            patch("skulk.worker.engines.mlx.cache.TURBOQUANT_V_BITS", 4),
        ):
            cache = make_kv_cache(model)
    ...
    def test_make_kv_cache_turboquant_adaptive_backend(self):
        ...
        with (
            ...
            patch("skulk.worker.engines.mlx.cache.TURBOQUANT_K_BITS", 3),
            patch("skulk.worker.engines.mlx.cache.TURBOQUANT_V_BITS", 4),
            patch("skulk.worker.engines.mlx.cache.TURBOQUANT_FP16_LAYERS", 1),
        ):
            cache = make_kv_cache(model)
...
def _load_distributed_test_model() -> tuple[Model, object]:
    ...
    from skulk.worker.engines.mlx.utils_mlx import load_tokenizer_for_model_id
...
class TestKVPrefixCacheWithModel:
    ...
    def test_evicts_lru_entry_under_memory_pressure(self, model_and_tokenizer):
        ...
        with patch(
            "skulk.worker.engines.mlx.cache.get_memory_used_percentage",
            return_value=0.95,
        ):
