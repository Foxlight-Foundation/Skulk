...
# with an older revision loses its override power against a signed or installed
# card for the same id (that card is current truth), while a card with NO stamp
...
# Whether the empty-catalog warning has been logged for the current cause.
_empty_catalog_explained = False
...
async def _load_cards_from_registry() -> bool:
    ...
    try:
        ...
        logger.warning(
            f"signed model registry unavailable ({error}); the catalog holds "
            "installed and custom models until it answers"
        )
        _registry_current_cards.clear()
        _registry_engine_support = ()
...
async def _load_cards_from_dir(directory: Path, *, is_custom: bool) -> None:
    ...
    Within a load pass the first card for a model id wins (duplicate ids
    among custom files keep their existing precedence). A current CUSTOM file
    replaces both a signed or installed card and retained custom sidecar
    metadata for the same id: the custom directory exists for operator
    override, and a first-wins rule would silently ignore the override because
    those cards load first (#652). One exception: a MACHINE-GENERATED custom card (stamped
    ...
    live" while the signed or installed card actually serves.
    """
    ...
    async for toml_file in directory.rglob("*.toml"):
        try:
            ...
            existing = _card_cache.get(card.model_id)
            if is_custom:
                # A generated card keeps the curated limit on every reload, not
                # only when it was added: the signed or installed card for the
                # same id, else the repository's strictest signed alias.
                card = preserve_generated_card_constraints(
                    card,
                    existing
            ...
            if existing is None:
                ...
                if stale_generated:
                    logger.warning(
                        ...
                        f"older Skulk (generator revision "
                        f"{card.generator_revision} < {CARD_GENERATOR_REVISION}"
                        f") and is superseded by the signed or installed card for "
                    ...
                    logger.info(
                        f"custom model card {toml_file} overrides the signed or installed "
...
async def _refresh_card_cache() -> None:
    ...
    _explain_an_empty_catalog(registry_loaded)


def _explain_an_empty_catalog(registry_loaded: bool) -> None:
    """Say why the catalog is empty, once per cause, instead of failing silently.

    Skulk ships no model cards. A card comes with the model: from the signed
    registry when a model is downloaded, then from the model's own record.
    A node that has never reached the registry, and holds no installed or
    custom model, therefore has nothing to list, and the operator needs to
    know that is why.
    """

    global _empty_catalog_explained  # noqa: PLW0603
    if _card_cache or registry_loaded:
        _empty_catalog_explained = False
        return
...
def _apply_installed_card_snapshot(
    ...
    for model_id, record in selected_records.items():
        ...
        if record.model_card.is_custom:
            ...
            # must not resurrect the card or shadow registry truth.
...
def unregister_installed_card_record(model_id: ModelId) -> None:
    ...
    Side effects:
        ...
        cache for a registry/installed/custom rebuild on its next read.
    """
    ...
    async def load(model_id: ModelId) -> "ModelCard":
        ...
        Args:
            model_id: Exact selectable model alias already present in the signed,
                installed, or operator-added catalog.
        ...
        *,
        ...
        return ModelCard(
            ...
            placement=PlacementCardConfig(
                # Both llama.cpp engines, mirroring the curated GGUF text
...
def preserve_generated_card_constraints(
    ...
    Generated cards are metadata caches, so they must not erase a curated
    ...
    if (
        ...
        or curated is None
        or curated.is_custom
        or curated.placement.max_pipeline_split_layer is None
    ):
        return generated
    placement = generated.placement.model_copy(
        update={"max_pipeline_split_layer": curated.placement.max_pipeline_split_layer}
    )
...
async def delete_custom_card(model_id: ModelId) -> bool:
    ...
    startup, so a deleted override falls back to the signed or installed card
    for the model, or leaves the catalog when neither exists.
    """
