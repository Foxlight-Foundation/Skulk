...
def test_comfy_root_without_interpreter_is_loud() -> None:
    facts = make_facts(gpus=(NVIDIA_A40,)).model_copy(update={"comfy_root": "/opt/ComfyUI", "comfy_root_state": "ok"})
    derivation = derive_node_backends(facts)
    assert "comfy" not in derivation.backends
    assert any(c.code == "invalid_engine_binary" and "SKULK_COMFY_ROOT is set" in c.message for c in derivation.conflicts)
