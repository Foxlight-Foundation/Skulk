...
def test_generation_stats_redaction_scopes_batching_truth_to_benchmarks() -> None:
    ...
    # Ordinary chat streams expose no runner attribution or live admission state.
    ...
    benchmark = stats.redacted_for_benchmark_client()
    # The explicit benchmark endpoint retains only the non-identifying truth
    # needed to prove real batching.
    assert benchmark.serving_node is None
    assert benchmark.serving_backend is None
    assert benchmark.in_flight_at_admission == 5
    assert benchmark.serving_batches is True
