...
def _primary_vulkan_override(env: Mapping[str, str]) -> Path | None:
    """Return a pinned standalone primary binary that exposes a Vulkan device."""
    configured = env.get(AUDIO_CPP_BIN_ENV, "").strip()
    binary = Path(configured) if configured else None
    if (
        binary is None
        or not binary.is_file()
        or not os.access(binary, os.X_OK)
        or verified_cached_audio_cpp_binary(binary)
    ):
        return None
    try:
        audio_cpp_model_specs(binary, environ=env)
        from skulk.facts.probe import probe_audio_cpp

        if "vulkan" in probe_audio_cpp(str(binary)).computes:
            return binary
    except (OSError, RuntimeError):
        # An invalid primary override must not prevent the pinned Vulkan
        # package from being prepared for a valid claim.
        pass
    return None
...
def rehydrate_cached_audio_cpp(
    *, environ: Mapping[str, str] | None = None
    ...
    for variant, variable in variants:
        ...
        if variant == "vulkan" and primary_vulkan is not None:
            continue  # the explicit primary binary also owns this lane
        try:
            wheel = _wheel_for_variant(variant)
...
def prepare_audio_cpp(
    *, allow_download: bool,
    ...
    if variant == "vulkan" and not explicit and (
        primary_path := _primary_vulkan_override(env)
    ):
        return primary_path
    if explicit:
        path = Path(explicit)
