...
def build_sidecar_path(model_id: ModelId, sidecar_filename: str) -> Path | None:
    ...
    reject their directories. Searches ``SKULK_MODELS_PATH`` and then
    ``SKULK_MODELS_DIR`` for ``<org--repo>/<sidecar_filename>``.
...
def build_companion_model_path(model_id: ModelId) -> Path | None:
    ...
    Searches ``SKULK_MODELS_PATH`` then ``SKULK_MODELS_DIR``.
...
async def ensure_models_dir() -> Path:
    await aios.makedirs(SKULK_MODELS_DIR, exist_ok=True)
    return SKULK_MODELS_DIR
...
async def seed_models(seed_dir: str | Path):
    """Move models from resources folder to SKULK_MODELS_DIR."""
