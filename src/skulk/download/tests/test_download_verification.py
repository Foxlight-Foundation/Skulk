...
from skulk.shared.types.worker.downloads import FileListEntry, RepoFileDownloadProgress
...
async def temp_models_dir(tmp_path: Path) -> AsyncIterator[Path]:
    ...
    with patch("skulk.download.download_utils.SKULK_MODELS_DIR", models_dir):
        yield models_dir
...
class TestFileVerification:
    ...
    async def test_redownload_when_file_size_changes_upstream(
        ...
        with (
            ...
            with patch(
                "skulk.download.download_utils.calc_hash",
                new_callable=AsyncMock,
...
class TestFileListCache:
    ...
    async def test_fetch_fresh_and_update_cache(
        ...
        with (
            patch("skulk.download.download_utils.SKULK_MODELS_DIR", models_dir),
            patch(
                "skulk.download.download_utils.fetch_file_list_with_retry",
                new_callable=AsyncMock,
