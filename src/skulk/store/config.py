...
Overview
--------
This module defines the Pydantic models for ``skulk.yaml``, the optional
cluster-level configuration file for Skulk.  The file is placed alongside
the project root on **every node** in the cluster.  If the file is absent,
Skulk behaves identically to the zero-config default.
...
--------------------------
Without a model store every node independently downloads its assigned
...
---------------------------
``skulk.yaml`` must be placed at the **same relative path on every node**
(alongside the project root).  Node-specific behaviour is handled via
...
class LoggingConfig(FrozenModel):
    ...
    When ``enabled`` is ``True`` and ``ingest_url`` is set, Skulk emits
...
def load_skulk_config(
    ...
    if raw is None:
        return None
    return SkulkConfig.model_validate(raw)
