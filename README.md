...
- **Concurrent serving on the served llama.cpp engine, with dynamic context.** The `llama_server` engine keeps up to 16 generations in flight by default, and each instance's context is sized dynamically from what actually fits in memory beside the weights rather than a fixed cap. A unified KV cache gives every slot the full stamped window while FIFO prompt-plus-output reservations keep aggregate occupancy inside the shared pool. **Why it matters:** GPU Linux nodes handle overlapping requests and long contexts without either head-of-line blocking, silently shortened contexts, or a mid-stream allocator kill.
...
<p align="center">
  <img src="docs/imgs/dash-5.png" alt="Skulk observability panel showing a distributed trace across three ranks" width="80%" />
</p>
