...
named sequence, or the named artifact family, with `--platform plain` naming
the listing that has no artifact family; two listings at one sequence must be
told apart with `--platform`), prints the listing as consent facts and
