...
| `SKULK_DATA_REORDER_BUFFER` | Default ON. When falsy (`0`/`false`/`no`) the API does NOT buffer/reorder data-plane output by `sequence`; it dispatches chunks in arrival order, relying on the transport's ordering (#279 Phase 3). Safe only when the DATA plane is Zenoh (per-publisher FIFO); the gossipsub default reorders, so leave it on there. Used to run the Phase 3 buffer-disabled coherence acceptance matrix before the reorder machinery is deleted. Read in `API.__init__` (`_reorder_buffer_enabled`). |
