...
- **Provider streaming contract (Phase 3 foundation):** `extensions/streams.py` is transport-independent. `CapabilityStreamFrame` keys one logical stream by `(call_id, direction)`, uses the shared `started/chunk/completed/failed/cancelled` kinds, starts at sequence 0, and carries structured payload, optional `InlineMediaAttachment` raw bytes, optional staged `BlobMediaAttachment`, or typed `CapabilityStreamError` (`provider_error / transport_error / unreachable / cancelled / timeout / invalid_frame`). `encode_capability_stream_frame` serializes a canonical JSON header and returns raw inline bytes separately; bytes in structured payloads are rejected. `CapabilityStreamReceiver` enforces bounded ordering, one terminal, duplicate idempotency, late-frame rejection, gap/idle deadlines, caller cancellation, and receiver-synthesized typed terminals. `CapabilityStreamHandler` is registered for server-streaming/bidirectional descriptors by `LoadedExtensions.stream_handler`. TTS server streaming is the planned first transport consumer; realtime STT follows with caller-direction half-close. No final acknowledgement in v1.
