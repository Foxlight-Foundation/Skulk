...
**Connectivity readings stay on the control plane.** `node_network`, `node_thunderbolt`, `node_thunderbolt_bridge`, and the derived `thunderbolt_bridge_cycles` are NOT telemetry: `apply()` builds the RDMA topology graph from `node_thunderbolt` (`MacThunderboltConnections` to `replace_all_out_rdma_connections`) and recomputes TB-bridge cycles from `node_network` + `node_thunderbolt_bridge`, and the planner reads `node_network` for host selection. Those define the graph placement runs on, so they must be ordered event-sourced state, not an unordered last-write-wins plane (#279 slice 3 scoping; refines the original "all of `NodeGatheredInfo` to telemetry" target). Because they stay on the ordered log AND double as the node's liveness heartbeat (`apply` bumps `last_seen` on every `NodeGatheredInfo`), they are **change-gated, not emitted every poll**: the worker forwards a connectivity reading only when its payload changed or every `_CONNECTIVITY_KEEPALIVE_SECONDS` (15s, under the 30s prune) as a keepalive (`worker/main.py:_forward_info`), and the master **de-dups by payload** in its index loop (`master/main.py`): an unchanged reading bumps `last_seen` (via `model_copy`, leaving the event-log idx sequence untouched) but is **not** appended/broadcast/snapshotted. Without this, connectivity churn (~2/s per fleet, worsened by unsorted `psutil` interface order on Linux, now sorted in `get_network_interfaces`) filled the master's 10k-event `REPLAY_TAIL_RETENTION_EVENTS` in ~83 min; a joining/failing-over node then replayed that burst and saturated the AMD nodes' bounded (1024) gossipsub send queues into a flap livelock (the 5-node gossip storm; same shape as the #278/#364 storms). Liveness stays correct: the master bumps `last_seen` from every reading it receives and prunes on it; only the master prunes, so a follower not seeing keepalive-only bumps is benign, and a re-elected master rebuilds `last_seen` from live events (its de-dup cache is empty, so the first reading per node after election is a cache miss and is logged + re-broadcast).
