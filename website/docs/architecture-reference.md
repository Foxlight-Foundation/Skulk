...
`PrepareAudioCpp` targets a worker selected from live `NodeResources.architecture`, platform class, participation, and memory. The master indexes `AudioCppPreparationRequested` and `AudioCppPreparationCompleted`; the request carries a five-minute expiry so replay cannot trigger stale package downloads. The worker prepares the pinned wheel (or a verified offline cache), refreshes facts, and publishes ready resources. Successful completion carries that verified resource snapshot, which the master applies to its live view before broadcasting the event; this closes the telemetry ordering gap before placement, including for a package the API already saw as ready. The API checks the ordered completion snapshot directly, so a dropped telemetry update does not time out a successful preparation. Placement stamps `resolved_engine_build` on each music shard from the exact live `NodeResources.engine_builds` entry. The runner rehashes the executable and probes the selected device immediately before each sidecar start, refusing a changed build. CUDA, ROCm, and Vulkan audio.cpp lanes admit against the accelerator memory pool in API preflight and placement; Apple Metal and CPU use system RAM. Exact music placements reject `LlamaRpcInstance` and multiple runner shards, and require a ready build plus matching signed support claim. `MusicGeneration` is a distinct single-host task. `worker/runner/audio_cpp/` supervises one loopback server per model instance, serializes generation, translates only typed family options, and validates a bounded PCM WAV. A terminal `MusicChunk` manifest and node-addressed `OUTPUT_MEDIA` purpose `music` settle the API job together. `api/music_jobs.py` persists lifecycle records; a separate `VideoStore` root retains verified WAV bytes for up to 24 hours, capped at 64 MiB each. Six `/v1/music` HTTP operations expose creation, list, status, WAV content, cancellation, and deletion.
