...
Dense per-symbol fact-sheet for AI assistants and operators who prefer reference style. For narrative and design rationale, see [Architecture](architecture). Source paths and symbol names identify the implementation; line numbers are navigation hints and may move as code changes.
...
| `SKULK_VLLM_GPU_MEMORY_UTILIZATION` | Fraction of GPU VRAM `vllm serve` may use for weights + KV cache (`--gpu-memory-utilization`; `src/skulk/worker/runner/vllm/runner.py`). Default `0.90` (vLLM's own default); an unparseable or out-of-`(0, 1]` value falls back to the default. This is a node-local serving setting. |
