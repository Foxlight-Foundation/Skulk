...
| `TELEMETRY` | `NodeTelemetry` | `GatheredInfo` (`NodeResources` — participation role + backends; `MemoryUsage`/`MactopMetrics`+`MacmonMetrics` — per-node memory + system profile; `NodeDiskUsage`/`MiscData`/`StaticNodeInformation`/`RdmaCtlStatus` — disk + identity + rdma-ctl, slice 3) | Workers | All nodes (applied into `TelemetryView`) |
...
Hardening toward default-on (#308 + #309). Security (#308): the session sets a Zenoh `namespace` (`ZenohConfig.namespace`, a collision-resistant SHA-256 hash via `_derive_zenoh_namespace` of the exact token libp2p isolates on. `_libp2p_namespace_token` mirrors `swarm.rs`: `SKULK_LIBP2P_NAMESPACE` when set, else the `NETWORK_VERSION` default `v0.0.1` (the legacy `EXO_LIBP2P_NAMESPACE` is NOT read, since libp2p ignores it; deriving from a different source would split one libp2p cluster across two Zenoh namespaces). Neither the raw token (it seeds libp2p's private-network PSK in `swarm.rs`) nor the derived namespace (with no TLS it is itself the isolation value a peer could subscribe with) is ever logged; startup logs only a short non-routing fingerprint plus whether the override env was set) that transparently prefixes every key, so a foreign-namespace peer cannot read this fleet's `data`; and `SKULK_ZENOH_LISTEN` is required explicitly (no `0.0.0.0` default). TLS/ACL remain operator-configurable for untrusted links. Robustness (#309): the DATA plane has its own outbound loop (`Router._zenoh_networking_publish` draining a dedicated channel), so its `Block` backpressure can't stall the shared `_networking_publish` control loop; and `ZenohSession::publish`/`subscribe` clone the publisher `Arc` / check-and-release before the `declare`/`put` await, so the publishers mutex never spans a network put and concurrent per-command publishes don't serialize.
...
| `SKULK_ZENOH_DATA_PLANE` | Experimental, default OFF. When truthy (`1`/`true`/`yes`) the `DATA` topic (per-token output) rides an Eclipse Zenoh peer session instead of gossipsub; all other planes stay on libp2p. Read in `Node.create` (`src/skulk/main.py`), wired in `Router` (`uses_zenoh`). **Security (#308):** the session is **namespace-isolated** (keys prefixed by a segment that is a collision-resistant SHA-256 hash of the exact token libp2p isolates on (`SKULK_LIBP2P_NAMESPACE` when set, else the `NETWORK_VERSION` default `v0.0.1`; mirrors `swarm.rs`, not the legacy `EXO_LIBP2P_NAMESPACE`). Neither the raw token nor the derived namespace is logged (with no TLS the namespace is the only isolation value); startup logs only a short non-routing fingerprint), so a peer on a different namespace cannot read this fleet's `data` (parity with the libp2p private namespace). There is still **no transport auth/TLS** by default, so on an untrusted network either enable Zenoh TLS (operator-configured) or keep it firewalled; a loud startup warning fires when on. |
