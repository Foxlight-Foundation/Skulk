...
- **Provider streaming (Phase 3, server-output slice):** `extensions/streams.py` is the transport-independent contract. `CapabilityStreamFrame` keys one logical stream by `(call_id, direction)`, uses shared `started/chunk/completed/failed/cancelled` kinds, and carries structured payload, optional raw `InlineMediaAttachment` (1 MiB/frame cap), optional staged `BlobMediaAttachment`, or typed `CapabilityStreamError`. `CapabilityStreamReceiver` enforces bounded ordering, one terminal, duplicate idempotency, late-frame rejection, five-second gap expiry, call/idle deadlines, and synthesized failures. A `CapabilityStreamHandler` registers only for `server_streaming` descriptors in this slice; Skulk emits sequence-zero `started`, then validates the handler's exact sequence, identity, `output_chunk_schema`, and one terminal. `ExtensionContext.stream_capability(...)` returns `CapabilityStreamSession(open_result, frames)`. Local/remote opening uses `POST /v1/capabilities/stream` as a control-sized admission request; output uses the distinct `PROVIDER_DATA` topic (not the `DataChunk` union), encoded by `routing/provider_streams.py` as a four-byte JSON-header length + canonical header + optional raw bytes. `PROVIDER_DATA` shares the Zenoh DATA scheduler: owner key `provider_data/<caller_node>`, same-node short circuit, independent bounded queues keyed by `(owner, call_id)`, owner/process admission caps, publish deadline, typed saturation rejection, and aggregate egress diagnostics. Early iterator close calls `POST /v1/capabilities/stream/cancel`; caller identity is checked and cancellation is idempotent. Client-streaming/bidirectional descriptors remain discovery-only until realtime STT input frames + half-close land. No final acknowledgement in v1; master/State/event log are absent from the path.
