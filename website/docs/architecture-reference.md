...
**Download progress admission.** Raw repository download callbacks occur once per 8 MiB chunk across parallel files, but `download_shard` retains only the latest callback per file behind one emitter task and drains that task before publishing terminal repository progress. `DownloadCoordinator` then applies a monotonic per-attempt fraction high-water mark (5% steps, one-second floor, 30-second heartbeat); lifecycle code explicitly resets the mark for a new attempt, while a lower fraction inside an attempt is stale and suppressed. Ordered `NodeDownloadProgress` events omit nested file details (`include_files=False`) and carry only aggregate progress. This keeps the event count and payload size bounded while preserving the state fields used by placement, workers, and the dashboard. The gossipsub publisher reports all-peer queue drops at most once per ten seconds without payload content; the PyO3 logging bridge suppresses libp2p-gossipsub's warning-level full-RPC dumps, which otherwise duplicate entire failed payloads into logs under pressure.
