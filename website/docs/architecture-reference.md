...
- **Context:** `ExtensionContext` = node_id + skulk_version + `embed_texts` (in-process equivalent of `POST /v1/embeddings`, backed by `API.embed_texts`; returns `None` when no embedding instance is placed) + `read_cluster()` (fabric-citizenship Phase 1: telemetry-plane read surface; returns an immutable `tuple[ClusterNodeView, ...]` snapshotting `TelemetryView` per node: node_id, friendly_name, backends, participation, skulk_version, accelerator_vendor, ram_total_bytes, last_telemetry; pure/in-memory, no mutation, fields `None` until each reading arrives; `extensions/telemetry.py:snapshot_cluster`). The advertise half (a plugin publishing its own capability onto telemetry) is a later slice: it is a wire-schema change (a new `GatheredInfo` variant in `TELEMETRY_PLANE_INFO`).
