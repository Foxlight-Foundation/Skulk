...
- **Built-in realtime STT provider (Phase 4 second consumer):** `BuiltinSpeechProvider` also reserves `stt.realtime@1.0.0`, a `bidirectional` mono PCM16-to-transcript contract. It is described unconditionally but advertised/admitted only with experimental mode + `experiments.stt_realtime` + a locally hosted mounted STT card declaring both `supports_streaming=true` and `supports_realtime=true`. Admission pins `RealtimeAudioTranscription` to that exact instance; the master creates the task without re-placement, transcript output follows normal owner-addressed DATA lifecycle, and caller PCM never enters State or the event log. A bounded Node-owned AnyIO channel carries API input to the local worker; a separate bounded multiprocessing channel carries worker input to the speech runner. The worker permits bounded pre-dispatch buffering, cancels overflow, and retains bounded finished-command tombstones so late frames cannot accumulate. The runner requires upstream `create_streaming_session`, linearly resamples mono PCM16 to the session input rate, emits progressive `TranscriptionChunk` deltas, half-closes and drains on caller completion, and cancels promptly. This first implementation deliberately requires API/worker/runner locality; remote serving-node ingress, `WS /v1/realtime`, VAD, and speech diagnostics remain later work.
...
**Zenoh data-plane transport (soft default-on, #315).** The `DATA` and `PROVIDER_DATA` topics can ride an Eclipse Zenoh peer session instead of gossipsub; control, telemetry, and election planes stay on libp2p. Transport selection is resolved by `_resolve_zenoh_enabled(SKULK_ZENOH_DATA_PLANE, SKULK_ZENOH_LISTEN)`: explicit `SKULK_ZENOH_DATA_PLANE` of `1`/`true`/`yes`/`on` forces Zenoh on (and still requires an explicit listen, #308), `0`/`false`/`no`/`off` forces gossipsub, and **unset** is soft default-on (Zenoh when `SKULK_ZENOH_LISTEN` is configured, else gossipsub) so a bare node with no Zenoh config stays on gossipsub instead of failing the #308 listen requirement. The `Router` holds an optional `ZenohHandle` (`skulk_pyo3_bindings`, backed by `rust/networking/src/zenoh_session.rs`) and routes both owner-addressed DATA families through its dedicated egress/receive loops. The session is a Zenoh `peer` with multicast scouting OFF and gossip + explicit `SKULK_ZENOH_CONNECT` endpoints (the macOS-Local-Network-Privacy-safe posture), publishing `Reliable` + `Block` on a single priority so one producer's frames are FIFO per key. Model DATA keeps its transport-conditional reorder policy; provider DATA always applies `CapabilityStreamReceiver` because the generic contract requires bounded duplicate/reorder/gap handling independent of the selected transport.

Key-addressed unicast uses `data/<owner_node>` and each node subscribes only to its own suffix. The in-process networking channel carries an `OutboundPacket` with topic, routing key, command stream key, terminal marker, and serialized bytes. Same-node frames are published to the API before egress and short-circuit Zenoh entirely. Remote frames enter a bounded queue per `(owner, command_id)` (`_ZENOH_DATA_STREAM_BUFFER`, with per-owner and process stream caps); every command has an independent publish task and five-second publish deadline. Queue saturation or publish failure drops only that command's frame, which the receiver detects as a sequence/idle transport failure. It never blocks unrelated same-node or remote streams. Terminal delivery closes the command worker. On gossipsub `owner_node` remains broadcast metadata and the API reorder path applies.
