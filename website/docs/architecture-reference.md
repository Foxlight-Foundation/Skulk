...
**Adaptive placement and cluster model trust (#845).** Repository-code trust is model-scoped, not node-scoped. `State.model_trust_approved_remote_code_identities` contains exact signed `card_...` or content-derived custom `local_...` identities approved once by the operator; the master orders changes through `SetModelTrustApproval`/`ModelTrustApprovalChanged`, and each node persists the replicated set under `SkulkConfig.model_trust` for offline runner enforcement. Worker runner-creation and load gates consume replicated State directly, so a failed persistence attempt cannot make a revoked stale-YAML approval executable. Revision-pinned Foxlight-provenance cards need no entry. An unapproved agent/community/custom/unsigned repository-code card fails before placement with `model_code_approval_required`; after approval it participates in the ordinary open `backend_preference`, download-locality, and memory ranking without a trust-based node filter. An unavailable first engine choice therefore falls through to the next admissible node/backend without a client selecting or reserving a preview. `GET /instance/previews` and `POST /place_instance` invoke the same planner against current facts; previews expose `error_code`, while placement errors retain readable `error.message` and add `X-Skulk-Placement-Failure` (`no_valid_placement`, `placement_info_pending`, or `model_code_approval_required`). Caller-specified `POST /instance` additionally rejects embedded shard cards whose model alias differs from the assignment with `model_card_identity_mismatch`. Backend and hardware identifiers remain open strings, so new engines do not require a wire-contract enum change.

**Model-trust operator boundary.** `POST|DELETE /models/remote-code-approvals/{card_id}` and `POST /models/add` require either a direct loopback request with no proxy-shaped headers or an authenticated operator-gateway request carrying `operations:write`. `PUT /config` never accepts a replaceable `model_trust` snapshot: after authenticating the caller it returns `409` and directs the operator to the dedicated master-ordered endpoints. `OperatorGatewayAuthorization` stamps an internal ASGI scope marker only after bearer validation; canonical handlers never trust a caller-provided header. Config broadcasts omit `hf_token` and `model_trust`; `DownloadCoordinator` merges the recipient's existing local token and state-derived trust before an atomic mode-`0o600` replacement. Custom cards always derive `local_...` trust identities from their complete effective content even if copied metadata includes a registry card id.

**Headless exact-card qualification.** `POST /models/add-card` and `DELETE /models/custom/{model_id}` additionally accept the exact high-entropy bearer configured as `SKULK_EXACT_CARD_QUALIFICATION_TOKEN`; no other handler accepts that service credential. Comparison is constant-time and values shorter than 32 characters are never active. The add path requires a full immutable source revision, clears every `registry_*` trust field, and assigns the persisted `qualification_only` ownership marker only to service-authenticated installs. A service caller cannot replace or delete any pre-existing non-qualification card; only loopback or an authenticated operator may manage operator-owned exact or custom cards. Service cleanup supplies the complete original candidate card, and the command carries that expected unsigned card to the elected master, which requires exact equality against its serialized ordered view before emitting a delete event. An older job therefore cannot delete a newer qualification card that reused the alias. Older indexed echoes never rewrite newer command-time truth; a promoted master lazily seeds a fresh view from its converged catalog, and later signed-registry refreshes supersede stale signed or qualification-owned entries without displacing operator custom truth. Add and service-cleanup success are withheld until that command ID's exact event has persisted and converged into the responding API catalog, so pre-existing state cannot acknowledge a retry; conflicts and timeouts fail explicitly. Downloaded artifacts remain durable after cleanup, but installed records carrying `qualification_only` are not projected back into the catalog after the lifecycle custom file is removed. The temporary card remains unsigned/custom and repository code still requires its ordinary model-level trust decision. `qualification_only` is excluded from content-derived remote-code identity because it is lifecycle ownership rather than executable artifact truth.
...
| `FailInstance` | Worker reports a terminal runner or model-trust failure with a stable category and bounded payload-free explanation. | Emit `InstanceFailureRecorded` before the normal `InstanceDeleted` teardown; an unknown/already-removed instance is an idempotent no-op. |
...
| `/models/add` | POST | Register a custom model card through direct loopback or the authenticated operator gateway. Optional `gguf_file` pins one exact quant from a multi-quant GGUF repo: `ModelCard.fetch_from_hf(model_id, gguf_file=...)` -> `select_requested_gguf` (repo-membership-verified) instead of `select_preferred_gguf`; the pin is carried by `StoreDownloadRequest.gguf_file` on `POST /store/models/{id}/download`, and staging fast paths reject a staged dir missing the pinned quant or its shard group (`_staged_pinned_gguf_missing`, store-side recovery before staging). |
| `/models/add-card` | POST | Persist one complete exact card through direct loopback or the authenticated operator gateway. Preserves artifact pins but forces `is_custom=true` and removes registry card, snapshot, and provenance claims so pre-publication qualification remains unsigned and model-level remote-code approval still applies. |
| `/models/custom/{model_id}` | DELETE | Remove a custom card |
| `/instance` | POST | Place a fully specified `CreateInstanceParams`; returns the accepted `command_id`, exact submitted `instance_id`, and model card so clients can correlate the acknowledgement with runtime and failure truth. |
| `/place_instance` | POST | Place a model: master picks ranks. Takes `PlaceInstanceParams` (model id + placement preferences, optional `excluded_nodes: list[NodeId]` to exclude specific nodes from this placement) and returns the accepted `command_id` plus its exact `instance_id`; not interchangeable with `/instance`, which takes a fully-specified `CreateInstanceParams`. |
...
`src/skulk/provisioning/manifest.py`, currently `b10434`) so upstream churn is
