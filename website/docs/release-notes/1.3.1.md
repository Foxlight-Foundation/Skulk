...
- **Complete, capability-accurate model cards for every model in the store.** Every
  model now has a card whose properties are derived from the model's real
  capabilities, so `/v1/models` metadata, reasoning defaults, prompt rendering, and
  tool-call handling match what each model can actually do. Speculative-decoding
  companions (a separate draft model, or an MTP prediction-head sidecar) are marked
  as such and are no longer offered as standalone launchable models.

- **The dashboard renders AMD Ryzen AI Max (Strix Halo) nodes as their own device.**
  The topology graph and cluster cards draw a dedicated AMD glyph and report the
  node's full unified memory (VRAM carve-out plus system RAM), and served
  (llama.cpp / `llama-server`) instances now show the correct engine label and their
  MTP badge instead of a generic pipeline label.
...
- **The bundled Qwen3.6-27B MTP model downloads reliably.** Its card now points at
  the well-provisioned `unsloth/Qwen3.6-27B-MTP-GGUF` mirror instead of a
  rate-limited community repo whose 17 GB weights could take hours and never finish
  landing in the store. The model is otherwise identical: same base weights, same
  native MTP heads, same served `--spec-type draft-mtp` path.
