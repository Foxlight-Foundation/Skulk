...
When the card also declares `audio.supports_streaming = true`, clients can pass
`stream=true` for chunked HTTP MP3 output; mounted `supports_transcription`
models serve non-streaming `/v1/audio/transcriptions`.
