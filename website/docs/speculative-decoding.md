...
roughly the cost of one. Because the verify step accepts or rejects
...
quality trade-off. (On some Qwen models the greedy text can differ
...
- **Multi-node MoE placements.** Sparse (mixture-of-experts) models like
  ...
  -7% (30.2 to 28.2 tok/s) on a 2-node pipeline. The card gates this with
  ...
  measures ~2.2x (16 to 35.1 tok/s).
