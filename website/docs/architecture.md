...
9. **Output streaming.** The rank-0 runner publishes each generated chunk on the `DATA` topic addressed to the node that owns the request, which drains it into that request's queue. The master does not index or relay per-token output (see [how the cluster communicates](cluster-communication) and the Data plane note above).
