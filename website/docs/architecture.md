...
- **libp2p pub/sub for transport.** Topics carry commands, events, telemetry, and connection updates between nodes. Election messages use a dedicated Python egress queue plus a second gossipsub behavior and protocol on the same libp2p swarm, isolating liveness traffic from bulk fan-out before and inside the transport. During protocol migration they are also copied over the default behavior for rolling-upgrade compatibility.
...
Download lifecycle still uses ordered `NodeDownloadProgress` events because workers and placement consume the terminal state. Intermediate repository callbacks are serialized through one bounded, per-download coalescer before they reach that path: only the latest callback per file is retained, regressions within one attempt are rejected against a monotonic high-water mark, and the coalescer drains before completion or failure. Store staging resolves the canonical registered byte total before transferring files, so fresh and resumed copies report one monotonic aggregate fraction rather than comparing multi-file downloaded bytes with the current file's size. Ordered progress carries aggregate bytes, file counts, speed, and ETA but omits the nested per-file map. Together with a fraction gate, this bounds intermediate event count independently of model size and guarantees that stale chunk callbacks cannot follow a terminal status. Non-terminal download progress moving entirely to the last-write-wins telemetry plane remains tracked in issue #279.
