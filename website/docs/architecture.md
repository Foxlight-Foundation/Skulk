...
in classifier-sized source slices and stop at the detected boundary. The socket serializes multiple
utterances as distinct provider calls with linked item IDs, per-turn VAD reset,
and no overlapping provider ownership. It does not generate model responses or
claim speech-to-speech orchestration. Every API
