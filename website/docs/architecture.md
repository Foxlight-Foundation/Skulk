...
### Memory content store (experimental)

Skulk carries an experimental associative-memory library in `src/skulk/memory`: holographic reduced representation primitives, a pattern-separation (whitening) stage, and a fixed-size memory field that superposes many key/value traces and answers cue probes with a confidence score. Nothing else in Skulk imports this package yet; it changes no runtime behavior.

Its one on-disk component is the content store (`src/skulk/memory/store.py`), a durable node-local record of captured spans under the node's data directory. The store, not the field, is the source of truth: each record holds the verbatim span text, its provenance (role, session, capturing node, timestamp), and the exact key and value vectors bound into the field, stored as base64 half-precision. Fields are rebuildable indexes over this history; `rebuild` replays the live records into a fresh field in the original write order, so losing a field to a crash or node death costs only decay recency, never memories.

The durability posture mirrors the event log. Records append to line-oriented JSONL segments with an fsync cadence and size-based rotation; a crash mid-append leaves a torn final line that reads tolerate and the next append repairs by truncating to the last complete record. Forgetting is exact: a tombstone marks the span, reads hide it immediately, and compaction rewrites the surviving records into a fresh segment under a name no earlier manifest generation ever referenced, with an atomic manifest swap as the single commit point, so a crash anywhere during compaction leaves either the complete old view or the complete new view. The store also refuses to be what fills a disk: a free-space floor and ENOSPC handling at every write site drop it into a degraded mode where capture stops and reads keep working, because memory must degrade to off while inference continues unaffected. The whitening stage persists alongside the segments so a rebuilt node reconstructs the exact cue space its stored keys were written in.
