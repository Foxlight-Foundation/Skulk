...
4. **Placement / runner discovery.** The API issues a command on `COMMANDS` topic that the master indexes. If no instance for the model is currently placed, placement happens here (master picks ranks based on memory and topology).
