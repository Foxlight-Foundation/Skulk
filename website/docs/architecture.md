...
- **Data plane** (`DATA` topic) carries generated token, image, embedding, transcription, and audio output. The serving rank-0 worker streams an explicit per-command lifecycle (`started`, payload `chunk` frames, then exactly one `completed`, `failed`, or `cancelled`) directly to the owning API node. Every frame has a monotonic sequence; the API repairs bounded gossipsub reordering, deduplicates Zenoh loopback, and turns any unresolved sequence gap into a terminal transport error instead of returning incomplete output. The master never indexes, persists, or relays these frames. Zenoh delivery is key-addressed to the owner and uses bounded independent per-command egress queues with a publish deadline, so a blocked owner or command can lose only its own stream and cannot stall same-node or unrelated remote work. Queue saturation is therefore an explicit fail-and-cancel policy, not global backpressure or silent corruption. Node diagnostics and the dashboard expose lifecycle counts, first-byte/span timing, ordering anomalies, queue depth, drops, and publish failures. Existing OpenAI audio edges retain base64/JSON compatibility; provider streaming reserves a JSON header plus raw inline media attachment (or staged blob reference) because framing measurements show base64 approaches 1.334x wire size for large audio while the binary shape approaches 1.001x. Inbound vision chunks (`InputChunkReceived`, low-volume) stay on the control plane for now. See [how the cluster communicates](cluster-communication) for transport and trust details.
...
- An extension can also be a **provider**: a plugin that serves a capability of
  ...
  share the DATA lifecycle vocabulary through `CapabilityStreamFrame`. Provider
  frames support structured JSON payloads, raw inline media beside the header,
  and staged immutable blob references; `CapabilityStreamReceiver` enforces
  sequence-zero start, bounded reordering, one terminal, idempotent terminal
  duplicates, cancellation, and receiver-synthesized timeout/transport failures.
  Transport wiring and the first TTS provider consumer build on this contract.
