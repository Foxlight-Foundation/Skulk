...
# a node without a skulk.yaml (#629). A fresh box therefore gets a
# single-node store: this host serves its own store from ~/.skulk. An
# existing skulk.yaml is never touched, so re-runs and operator configs win.
if [[ ! -f skulk.yaml ]]; then
    STORE_PATH="$HOME/.skulk/model-store"
    mkdir -p "$STORE_PATH"
    cat > skulk.yaml <<EOF
# Generated by install.sh: single-node model store defaults so the
# store-first download flow works out of the box. Safe to edit or delete.
# When joining multiple nodes into one cluster, designate exactly ONE store
# host and point every node's store_host at it (see the Model Store page in
# the docs).
model_store:
