...
and MiniMax Music 3 with CPU and Vulkan backends. It carries the two pinned
loader model specs and incorporated licenses. Model weights remain separate.

The runtime needs a working Vulkan loader and GPU driver on the host. A wheel
build or successful CPU probe does not establish usable Vulkan music generation.
Skulk publishes `audio_cpp-vulkan` only after a device probe, and a model can
